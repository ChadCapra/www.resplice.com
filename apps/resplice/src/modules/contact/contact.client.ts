...
import type { ContactStore } from '$modules/contact/contact.store'
import type { Contact, PendingContact } from '$modules/contact/contact.types'
...
export interface ContactsClient {
	...
	editDescription: (params: Pick<Contact, 'id' | 'description'>) => Promise<void>
	...
	addShare: (id: Contact['id'], attributeId: UserAttribute['id']) => Promise<void>
	removeShare: (id: Contact['id'], attributeId: UserAttribute['id']) => Promise<void>
...
function contactsClientFactory({
			...
			case ServerMessageType.CONTACT_STATE:
				...
				if (m.data.contactAttributes) {
					store.contactAttributes.update((state) =>
						processRecords(state, 'id', m.data.contactAttributes, m.data.expiredContactAttributeIds)
					)
				}

				if (m.data.contactShares) {
					store.contactShares.update((state) =>
						processRecords(state, 'id', m.data.contactShares, m.data.expiredContactShareIds)
					)
				}
				break
		}
	...
	return {
		...
		acceptPending: async (id) => {
			...
			const contact: Contact = await deserializeServerMessage(new Uint8Array(resBuffer), crypto)
