...
function splicesClientFactory(api: any): SplicesClient {
	return {
		...
		getMember: (spliceUUID, memberUUID) => api.getSpliceMember(spliceUUID, memberUUID),
		getMembers: (spliceUUID) => api.getSpliceMembers(spliceUUID),
		getAllMembers: () => api.getAllMembers(),
		removeMember: (spliceUUID, memberUUID) => api.removeSpliceMember(spliceUUID, memberUUID),
		createInvite: (spliceUUID) => api.createMemberInvite(spliceUUID),
		getShare: (spliceUUID, shareUUID) => api.getSpliceShare(spliceUUID, shareUUID),
		getShares: (spliceUUID) => api.getSpliceShares(spliceUUID),
		getAllShares: () => api.getAllSpliceShares(),
		addShare: (spliceUUID, attributeUUID) => api.addSpliceShare(spliceUUID, attributeUUID),
		removeShare: (spliceUUID, shareUUID) => api.removeSpliceShare(spliceUUID, shareUUID)
	}
}
