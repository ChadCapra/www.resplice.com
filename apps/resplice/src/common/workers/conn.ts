...
import { serializeClientMessage, deserializeServerMessage } from '$services/serde'
import {
	ConnCommandType,
	type ConnCommand,
	ConnMessageType,
	type ConnMessage,
	type ClientMessage
} from '$shared/commuters/connCommuter'
...
const commands$ = fromEvent<MessageEvent<ConnCommand>>(ctx, 'message').pipe(pluck('data'))
...
commands$
	...
	.subscribe((cmd: Extract<ConnCommand, { type: ConnCommandType.SEND }>) => send(cmd.message))

async function initializeSocket(wsEndpoint: string, crypto: ReCrypto, handshake: ClientMessage) {
	ctx.crypto = crypto
