...
function handleCacheError(e: any, type: Command, data?: any) {
	ctx.postMessage({
		type,
		...data,
		error: Error(`Something went wrong when querying to the cache: ${e.errorCode}`)
	})
}
...
function openIndexedDb() {
	...
	request.onerror = (e: any) => {
		...
		ctx.postMessage({
			type: Command.CONNECT,
			data: false,
			err: Error(`Something went wrong when connection to the cache: ${e.errorCode}`)
		})
	}
...
function readCache() {
	...
	transaction.oncomplete = () => ctx.postMessage({ type: Command.READ_CACHE, cache })
...
function addToStore(store: Store, key: string, data: any) {
	...
	transaction.oncomplete = () => ctx.postMessage({ type: Command.ADD_TO_STORE, store, key })
	transaction.onerror = (e) => handleCacheError(e, Command.ADD_TO_STORE, { store, key })
...
function readStore(store: Store, key?: string) {
	...
	transaction.onerror = (e) => handleCacheError(e, Command.READ_STORE, { store, key })
...
function updateStore(store: Store, key: string, data: any) {
	const transaction = db.transaction(store, 'readwrite')
	transaction.onerror = (e) => handleCacheError(e, Command.UPDATE_STORE, { store, key })
...
function removeFromStore(store: Store, key: string) {
	const transaction = db.transaction(store, 'readwrite')
	transaction.onerror = (e) => handleCacheError(e, Command.REMOVE_FROM_STORE, { store, key })
