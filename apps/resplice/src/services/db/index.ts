...
function read<T = Record<Store, any>>(store: Store): Promise<T>
function read<T = Record<Store, any>>(stores: Store[]): Promise<T>
function read<T = Record<Store, any>>(store: Store | Store[]) {
  return new Promise((resolve, reject) => {
...
function clear(): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const stores: Store[] = []
    for (let i: number; i < DOMStringStores.length; i++) {
